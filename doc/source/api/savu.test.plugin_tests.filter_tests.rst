savu.test.plugin_tests.filter_tests package
===========================================

.. automodule:: savu.test.plugin_tests.filter_tests
    :members:
    :private-members:
    :undoc-members:
    :show-inheritance:

Submodules
----------

.. toctree::

   savu.test.plugin_tests.filter_tests.background_estimator_test
   savu.test.plugin_tests.filter_tests.band_pass_test
   savu.test.plugin_tests.filter_tests.component_analysis_test
   savu.test.plugin_tests.filter_tests.denoise_bregman_test
   savu.test.plugin_tests.filter_tests.dezing_filter_test
   savu.test.plugin_tests.filter_tests.distortion_correction_test
   savu.test.plugin_tests.filter_tests.downsample_filter_test
   savu.test.plugin_tests.filter_tests.find_peaks_test
   savu.test.plugin_tests.filter_tests.fitting_test
   savu.test.plugin_tests.filter_tests.histogram_test
   savu.test.plugin_tests.filter_tests.median_filter_test
   savu.test.plugin_tests.filter_tests.monitor_correction_test
   savu.test.plugin_tests.filter_tests.paganin_test
   savu.test.plugin_tests.filter_tests.pyfai_test
   savu.test.plugin_tests.filter_tests.raven_filter_test
   savu.test.plugin_tests.filter_tests.ring_artefact_test
   savu.test.plugin_tests.filter_tests.sinogram_alignment_test
   savu.test.plugin_tests.filter_tests.strip_background_test
   savu.test.plugin_tests.filter_tests.vo_centering_test

