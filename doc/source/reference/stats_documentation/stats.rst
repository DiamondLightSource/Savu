...
.. automethod:: savu.plugins.filters.denoising.base_median_filter.BaseMedianFilter.get_max_frames

Here are examples of how this methid is already used in Savu:
...
If the process list included an iterative loop, there will also be a graph showing the **NRMSD** (normalised root mean square deviation) between adjacent iterations in the loop. This is a measure of how much the data is being changed by each iteration of the loop as whole, *not* by each plugin in the loop.
