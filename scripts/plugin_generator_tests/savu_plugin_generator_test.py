...
class SavuPluginGeneratorTest(unittest.TestCase):
    ...
    def check_directories(self):
        ...
        self.check_plugin_tools(savu_base_path, plugin_folder)
        self.check_plugin_documentation(savu_base_path, plugin_folder)
        #self.check_plugin_tests(savu_base_path, plugin_folder, plugin_coverage)


    def check_plugin_tools(self, savu_base_path, plugin_folder):
        """Check if there is a plugin tools file for each of the
        plugin files
        """
        plugin_tools_list = [p.split('_tools.py')[0] for p in plugin_folder
                             if 'tools' in p]
        plugin_list = [p.split('.py')[0] for p in plugin_folder
                       if 'tools' not in p]
    ...
    def check_plugin_documentation(self, savu_base_path, plugin_folder):
        """Check if there is a plugin documentation file for each of the
        plugin files
        """
        plugin_doc_list = tplu.get_all_files_from(savu_base_path
                                    + 'doc/source/documentation')

        plugin_list = [p.split('.py')[0] for p in plugin_folder
                       if 'tools' not in p]
        plugin_doc_list = [p.split('_doc.rst')[0] for p in plugin_doc_list]
