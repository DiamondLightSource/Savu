...
class CropProjections(Plugin, CpuPlugin):
    ...
    Default: 0.
    :u*param useRelativeSizes: If True, the image will be cropped by xcrop and\
     ycrop parameters. If False, the absolute xsize and ysize parameters will \
     be used to determine the new image size. Default: True.
    ...
    def setup(self):
        ...
        self.useRelativeSizes = self.parameters['useRelativeSizes']
        ...
        elif not self.useRelativeSizes:
            if self.xsize > 0:
                remainder = self.xsize % 2
                xslice = slice(self.shape[det_x] / 2 - self.xsize / 2, \
                self.shape[det_x] / 2 + self.xsize / 2 + remainder)
                self.shape[det_x] = 2 * (self.xsize / 2) + remainder
            else:
                xslice = slice(0, self.shape[det_x])
            if self.ysize > 0:
                remainder = self.ysize % 2
                yslice = slice(self.shape[det_y] / 2 - self.ysize / 2, \
                self.shape[det_y] / 2 + self.ysize / 2 + remainder)
                self.shape[det_y] = 2 * (self.ysize / 2) + remainder
            else:
