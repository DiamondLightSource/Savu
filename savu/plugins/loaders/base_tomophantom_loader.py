    ...
    def __get_backing_file(self, data_obj, file_name):
        ...
        for i in range(total_frames):
            ...
            if (file_name == 'synth_proj_data'):
                ...
                gen_data = TomoP3D.ModelSinoSub(self.tomo_model, proj_data_dims[1], proj_data_dims[2], proj_data_dims[1], (tmp.start, tmp.start+1), -self.angles, self.path_library3D)
            else:
                #generate phantom data
                gen_data = TomoP3D.ModelSub(self.tomo_model, proj_data_dims[1], (tmp.start, tmp.start+1), self.path_library3D)
