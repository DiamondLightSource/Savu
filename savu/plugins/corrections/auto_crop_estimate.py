...
class AutoCropEstimate(Plugin, CpuPlugin):
    ...
    :param threshold: A threshold to control the cropping strength . Default: 0.05.
    :param margin_skip: Skip number of pixels around the image border . Default: 10.    
    :param statbox_size: The size of the box to collect background statistics (mean) . Default: 20.
    :param increase_crop: Increase crop values to ensure more accurate cropping . Default: 20.
    :param out_datasets: 4x1 array with indices to crop . Default: ['indices2crop'].
    """
    ...
    def post_process(self):
        ...
        in_datasets[0].meta_data.set('crop_left', crop_left_horiz)
