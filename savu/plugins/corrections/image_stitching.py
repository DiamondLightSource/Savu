...
class ImageStitching(Plugin, CpuPlugin):
    ...
    def setup(self):
        ...
        self.c_top, self.c_bot, self.c_left, self.c_right = self.parameters['crop']
    ...
    def pre_process(self):
        ...
        if self.space == 'SINOGRAM':
            ...
            self.dark1 = 1.0 * \
                dark1[self.c_top: h_df // 2 - self.c_bot, self.c_left:]
            self.flat1 = 1.0 * \
                flat1[self.c_top: h_df // 2 - self.c_bot, self.c_left:]
    ...
    def process_frames(self, data):
        mat1 = data[0][self.sl1]
        mat2 = data[0][self.sl2]
        if self.space == 'SINOGRAM':
            ...
            mat2 = np.float32(mat2[self.c_top:self.height0 -
                                   self.c_bot, :self.width0 - self.c_right])
            if self.flat_use is True:
                flatdark1 = self.flat1 - self.dark1
                nmean = np.mean(np.abs(flatdark1))
                flatdark1[flatdark1 == 0.0] = nmean
                mat1 = (mat1 - self.dark1) / flatdark1
                flatdark2 = self.flat2 - self.dark2
                nmean = np.mean(np.abs(flatdark2))
                flatdark2[flatdark2 == 0.0] = nmean
                mat2 = (mat2 - self.dark2) / flatdark2
