...
class FastxrfFitting(BaseFilter, CpuPlugin):
    ...
    :param include_escape: Should we included escape peaks. Default: 1.
    ...
    def pre_process(self, exp):
        ...
        xrfd.paramdict["Experiment"]["incident_energy_keV"] = \
            mData.get_meta_data("mono_energy")/1000.
        ...
        xrfd.paramdict["FitParams"]["fitted_energy_range_keV"] = [2., 18.]
        xrfd.paramdict["FitParams"]["include_pileup"] = 1
        xrfd.paramdict["FitParams"]["include_escape"] = 1
        datadict = {}
        datadict["rows"] = self.get_max_frames()
        datadict["cols"] = 1
        datadict["average_spectrum"] = \
            mData.get_meta_data("average")
        datadict["Detectors"] = {}
        datadict["Detectors"]["type"] = 'Vortex_SDD_Xspress'
        xrfd.xrfdata(datadict)
        xrfd._createSpectraMatrix()
        #  I only know the output shape here!
    ...
    def setup(self, experiment):
        ...
        if I have these:
        for i,j in enumerate(xrfd.matrixdict['Descriptions'])
        :print i,j DICT -descriptions
        ...
        xrfd.paramdict["FitParams"]["background"] = \
            self.parameters['background']
        xrfd.paramdict["FitParams"]["fitted_energy_range_keV"] = \
            self.parameters['fit_range']
        xrfd.paramdict["FitParams"]["include_pileup"] = \
            self.parameters['include_pileup']
        xrfd.paramdict["FitParams"]["include_escape"] = \
        ...
        datadict["rows"] = 1
        datadict["Experiment"] = {}
        datadict["Experiment"]["incident_energy_keV"] = \
            mData.get_meta_data("mono_energy")/1000.
        datadict["Experiment"]["collection_time"] = 1.  # This isn't used!
        datadict["Detectors"] = {}
        datadict["Detectors"]["type"] = self.parameters['detector_type']
        dettype = datadict["Detectors"]["type"]
        npts = xrfd.paramdict['Detectors'][dettype]['no_of_pixels']
        ...
        spectrum_length = \
            len(xrfd.paramdict["FitParams"]["mca_channels_used"][0])
        ...
        characteristic_curves = xrfd.matrixdict["Total_Matrix"]
        out_d1.meta_data.set_meta_data('Characteristic_curves',
        ...
        out_d1.add_pattern("SPECTRUM_STACK",
                           core_dir=(-2, -1),
                           slice_dir=range(len(outshape)-2))
        out_d1.add_pattern("SPECTRUM",
                           core_dir=(-1,),
                           slice_dir=range(len(outshape)-1))
        out_d1.add_pattern("CHANNEL",
                           core_dir=(-2,),
                           slice_dir=range(len(outshape)-2).append(-1))
        ...
        if mData.get_meta_data("is_map"):
            ...
            for i in ndims:
                if i != projdir[0]:
                    if i != projdir[1]:
                        ovs.append(i)
            out_d1.add_pattern("PROJECTION", core_dir=projdir, slice_dir=ovs)
        if mData.get_meta_data("is_tomo"):
            ...
            out_d1.add_pattern("SINOGRAM",
                               core_dir=(rotation, projdir[-1]), slice_dir=ovs)
        # set frame chunk
        print outshape
