...
class TigreRecon(BaseRecon, GpuPlugin):
    ...
    :param blocksize: ossart and pocs algs, number of angles to be included in each iteration of proj and backproj. Default: 20.
    ...
    :param tv_iter: For algorithms ossart_tv, pocs that make use of a tvdenoising step in their iterations. Default: 50.
   ...
   #    Default: art_data_minimizing (Otherwise hardcoded, for now.)
    ...
    def pre_process(self):
        ...
        self.tigre_kwargs = dict(blocksize=self.parameters['blocksize'],
                                 verbose=self.parameters['verbose'],
                                 OrderStrategy=self.parameters['orderstrategy'],
                                 tviter=self.parameters['tv_iter'],
                                 numiter_tv=self.parameters['tv_iter'],
        ...
        print(np.shape(in_pData[0])[self.det_dimX_ind])
        print(np.shape(in_pData[0])[self.det_dimY_ind])
    ...
    def process_frames(self, data):
        ...
        self.det_dimX = np.shape(proj3d)[self.det_dimX_ind]
        self.det_dimY = np.shape(proj3d)[self.det_dimY_ind]
        #print(self.det_dimY)

        # Reconstruct 3D proj data with TIGRE
        geo = tigre.geometry(mode='parallel', nVoxel=np.array([self.det_dimY, self.vol_shape[0], self.vol_shape[0]]), default=True)
        geo.nDetector = np.array([self.det_dimY, self.det_dimX])
