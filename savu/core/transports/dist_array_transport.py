...
from savu.plugins.timeseries_field_corrections import TimeseriesFieldCorrections
...
class DistArrayTransport(TransportMechanism):
    ...
    def transport_run_plugin_list(self, exp):
        ...
        data_file_handler = pu.load_raw_data(input_file) #*** temporary data file handler: Quick fix
        data_file_handler.rotation_angle = \
            data_file_handler.rotation_angle[data_file_handler.image_key == 0] #*** moved this from timeseries_field_correction (not currently done for hdf5_transport)
        ...
        with closing(Context(targets=targets)) as context:
            ...
            for plugin_dict in plugin_list.plugin_list:
                ...
                [in_data, out_data] = self.create_data_object(context, in_data, out_data, previous_plugin, plugin)
                # TODO 
                #out_data = plugin.get_output_data(in_data)
                
                logging.debug("Starting processing  plugin %s", plugin_dict['id'])
                plugin.run_plugin(in_data, out_data, processes, process, data_file_handler, self)
        ...
        self.output_data(data_file_handler, in_data, plugin_list, processing_dir, group_name) 
    ...
    def output_data(self, data_file_handler, in_data, plugin_list, processing_dir, group_name):            
        #output_filename = self.output_plugin_list(data_file_handler, in_data, plugin_list, processing_dir)
    ...
    def output_plugin_list(self, data_file_handler, in_data, plugin_list, processing_dir):
        import time
    ...
    def create_data_object(self, context, in_data, out_data, previous_plugin, plugin):
        ...
        [in_data, out_data] = self.distribute_array(in_data, out_data, previous_plugin, plugin)
        return [in_data, out_data]

    def distribute_array(self, in_data, out_data, previous_plugin, plugin, dist=None):

        context = in_data.data.context
        
        if previous_plugin is not None:
            if previous_plugin.output_dist() is not plugin.input_dist():
                temp = in_data.data.toarray() #*** temporarily creating ndarray - read directly from the old distarray (create empty dist array and populate?)
        ...
        if isinstance(out_data, VolumeData):
            shape = (in_data.data.shape[2], in_data.data.shape[1], in_data.data.shape[2])
            out_data.shape = shape
        else:
            shape = in_data.data.shape #*** temporary fix
                   
        if isinstance(plugin, TimeseriesFieldCorrections):
            shape = ((shape[0] - len(in_data.image_key[in_data.image_key != 0])), shape[1], shape[2])         
        ...
        out_data.data = context.zeros(dist, dtype=np.int32) #*** load as 32 bit int array?
